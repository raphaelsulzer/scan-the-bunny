...
class Scanner:
    def __init__(self, alpha=30, n=10):
        ...
        # import a mesh
        self.path = "/home/adminlocal/PhD/PhD-Thesis-Harvard/python/scanning/data/airplane"
        # self.path = "/Users/mba222/GitHub/papers/PhD-Thesis-Harvard/python/scanning/data/bunny"
        ...
        # origin = np.array([-0.85,-0.85,0.80])
        origin = np.array([-0.65,-0.65,0.20])
        self.origin = np.tile(origin,(n,1))
    ...
    def scan(self):
        ...
        o3d.io.write_point_cloud(os.path.join(self.path,"pointcloud.ply"),pcd)
        np.savez(os.path.join(self.path,"pointcloud.npz"),points=points,sensor_position=spos)
    ...
    def scanVisualize(self,interactive=True,size=(600,600),texture=True):
        ...
        for i in tqdm(range(self.alpha)):
            ...
            vedo.io.screenshot(os.path.join(self.path, "img", "img{:03d}.png".format(i)))
            plt.close()
    ...
    def pointsVisualize(self,interactive=True,size=(900,600),with_mesh=False,duration=80):
        ...
        mesh = vedo.Mesh(os.path.join(self.path, self.model))
        mesh.texture(os.path.join(self.path, "0", "DefaultMaterial_baseColor.jpg"))
        mesh.lighting("default")
        # [default, metallic, plastic, shiny, glossy, ambient, off]
        ...
        for i in tqdm(range(self.alpha)):
            ...
            plt.show(interactive=interactive, size=size,axes=0,roll=0,
                     azimuth=-i*self.factor,resetcam=False)
            vedo.io.screenshot(os.path.join(self.path, name, "img{:03d}.png".format(i)))
            plt.close()

        for img in sorted(os.listdir(os.path.join(self.path, name))):
            im=Image.open(os.path.join(self.path, name, img))
            # im.show()
            # im=ImageOps.crop(im,(80,70,0,100))
            # im.show()
            images.append(im)
        images[0].save(os.path.join(self.path, name+".gif"),
                       save_all=True, append_images=images[1:], optimize=True, duration=duration, loop=0)

    def scanGIF(self,duration=0.1):
        print("\nMake GIFs\n")
        images = []
        for img in sorted(os.listdir(os.path.join(self.path, "img"))):
            images.append(imageio.imread(os.path.join(self.path, "img", img)))
        imageio.mimsave(os.path.join(self.path, "scanning.gif"), images, duration=duration)

        images = []
        for img in sorted(os.listdir(os.path.join(self.path,"pts"))):
            images.append(imageio.imread(os.path.join(self.path,"pts", img)))
        imageio.mimsave(os.path.join(self.path,"points.gif"), images, duration=duration)
    ...
    def scanGIF_PIL(self,duration=80):
        ...
        images = []
        for img in sorted(os.listdir(os.path.join(self.path, "img"))):
            im=Image.open(os.path.join(self.path, "img", img))
            # im.show()
            # im=ImageOps.crop(im,(80,70,0,100))
            # im.show()
            images.append(im)
        images[0].save(os.path.join(self.path, "scanning.gif"),
                       save_all=True, append_images=images[1:], optimize=True, duration=duration, loop=0)

        images = []
        for img in sorted(os.listdir(os.path.join(self.path, "pts"))):
            im=Image.open(os.path.join(self.path, "pts", img))
            # im.show()
            # im=ImageOps.crop(im,(80,70,0,100))
            # im.show()
            images.append(im)
        images[0].save(os.path.join(self.path, "points.gif"),
                       save_all=True, append_images=images[1:], optimize=True, duration=duration, loop=0)
    ...
    def gtVisualize(self,interactive=True,size=(600,600),lw=0.0,texture=False,duration=80):
        ...
        if texture:
            # mesh.texture(os.path.join(self.path, "0", "DefaultMaterial_baseColor.jpg"))
            mesh.texture(os.path.join(self.path, "bunny1ww-logo2.jpg"))
            name+="_tex"
            brightness = 1.0
        else:
            brightness = 2.5
        ...
        os.makedirs(os.path.join(self.path, "gt"), exist_ok=True)
        for i in tqdm(range(self.alpha)):
            ...
            plt.show(interactive=interactive, size=size, axes=0, roll=0,
                     azimuth=-i * self.factor, resetcam=False)
            vedo.io.screenshot(os.path.join(self.path, "gt", "img{:03d}.jpg".format(i)))
            plt.close()


            images.append(Image.open(os.path.join(self.path, "gt", "img{:03d}.jpg".format(i))))
    ...
    def meshVisualize(self,interactive=True,size=(600,600),lw=0.0,texture=False,duration=80):
        ...
        name = "mesh"
        sensor = self.origin[0]
        mesh = vedo.Mesh(os.path.join(self.path, "pointcloud_rt_5.0.ply"), c=[90, 90, 90])
        ...
        os.makedirs(os.path.join(self.path, "mesh"), exist_ok=True)
        for i in tqdm(range(self.alpha)):
            vedo.settings.useDepthPeeling = True
            ...
            plt.show(interactive=interactive, size=size, axes=0, roll=0,
                     azimuth=-i * self.factor, resetcam=False)
            vedo.io.screenshot(os.path.join(self.path, "mesh", "img{:03d}.png".format(i)))
            plt.close()


            images.append(Image.open(os.path.join(self.path, "mesh", "img{:03d}.png".format(i))))
            # images.append(imageio.imread(os.path.join(self.path,"mesh", "img{:03d}.png".format(i))))
    ...
    def pcVisualize(self,interactive=True,size=(600,600),with_mesh=False,duration=80):
        ...
        if with_mesh:
            mesht = vedo.Mesh(os.path.join(self.path, self.model))
            # mesh.texture(os.path.join(self.path, "0", "CH_NPC_MOB_Elephant_A01_MI_GRN_baseColor.png"))
            mesht.texture(os.path.join(self.path, "0", "DefaultMaterial_baseColor.jpg"))
            mesht.lighting("default")

        pc = vedo.load(os.path.join(self.path, "pointcloud.ply"))
        ...
        for i in tqdm(range(self.alpha)):
            ...
            images.append(Image.open(os.path.join(self.path, name, "img{:03d}.png".format(i))))
